---
title             : "Epidemiologische situatie COVID-19 in Nederland - `r format((Sys.Date()),'%d')` `r format((Sys.Date()),'%B')` 2022"
...
author: 
  - name          : "Marino van Zelst"
    ...
    email         : "marino.vanzelst@wur.nl"

affiliation:
  - id            : "1"
    institution   : "Vragen over deze rapportage kunnen verstuurd worden aan Marino van Zelst, twitter.com/mzelst. E-mail: marino.vanzelst@wur.nl"
...
Tot en met `r Sys.Date()` zijn er in Nederland in totaal `r last(all.data$cases)` COVID-19 patiënten gemeld aan het RIVM. Tot nu toe zijn er `r last(all.data$deaths)` mensen overleden.
...
gemeentegrenzen@data$id <- c(1:345)
...
colnames(cbs.data.plot) <- c("Municipality_code_2022","Besmettingen (sinds gisteren)","Besmettingen (7 dagen)")
...
gemeentegrenzen@data <- gemeentegrenzen@data %>%
  left_join(cbs.data.plot,by=c("Municipality_code_2022"))
...
province_datalist <- list(province.cases.today,province.cases.weekago,province.death.today,province.death.weekago)
...
colnames(province.data) <- c("statnaam","cases_today","cases_lastweek","death_today","death_lastweek")
...
require(kableExtra)
province.data.table <- province.data[,c(2,8,10,9,11)]
colnames(province.data.table) <- c("Provincie","Totaal","/100000","Totaal","/100000")

footnote.provincie <- paste0("Aantal bij de GGD’en gemelde COVID-19 patiënten en overleden COVID-19 patiënten per provincie van ",format((Sys.Date()-14),"%d")," ",format((Sys.Date()-14),"%B"), " t/m ",format((Sys.Date()),"%d")," ",format((Sys.Date()),"%B")," 10:00 uur, totaal en per 100.000 inwoners")

kable(province.data.table, "latex", booktabs = T, linesep="") %>%
kable_styling() %>%
add_header_above(c(" " = 1, "Besmettingen" = 2, "Overleden" = 2)) %>%
...
ggd_datalist <- list(ggd.cases.today,ggd.cases.weekago,ggd.death.today,ggd.death.weekago)
...
colnames(ggd.data) <- c("statnaam","cases_today","cases_lastweek","death_today","death_lastweek")
...
require(kableExtra)
ggd.data.table <- ggd.data[,c(2,8,10,9,11)]
colnames(ggd.data.table) <- c("GGD","Totaal","/100000","Totaal","/100000")

footnote.ggd <- paste0("Aantal bij de GGD’en gemelde COVID-19 patiënten en overleden COVID-19 patiënten per GGD van ",format((Sys.Date()-14),"%d")," ",format((Sys.Date()-14),"%B"), " t/m ",format((Sys.Date()),"%d")," ",format((Sys.Date()),"%B")," 10:00 uur, totaal en per 100.000 inwoners")
...
agegroup_datalist <- list(agegroup.cases.today,agegroup.cases.weekago,agegroup.death.today,agegroup.death.weekago)
...
colnames(agegroup.data) <- c("statnaam","cases_today","cases_lastweek","death_today","death_lastweek")
...
agegroup.data.table <- agegroup.data[,c(1,7,9,8,10)]
colnames(agegroup.data.table) <- c("Leeftijdsgroep","Totaal","%","Totaal","%")

footnote.leeftijd <- paste0("Leeftijdsverdeling van bij de GGD’en gemelde COVID-19 patiënten en overleden COVID-19 patiënten van ",format((Sys.Date()-14),"%d")," ",format((Sys.Date()-14),"%B"), " t/m ",format((Sys.Date()),"%d")," ",format((Sys.Date()),"%B")," 10:00 uur.")

footnote.under50 <- "Overlijdensgevallen van patiënten jonger dan 50 jaar oud worden door het RIVM in de casusdata gegroepeerd in de categorie <50. Deze overlijdensgevallen zijn hier dus niet zichtbaar in deze leeftijdsgroepen maar alleen via de groep '<50'."

kable(agegroup.data.table, "latex", booktabs = T, linesep="") %>%
kable_styling(font_size = 11) %>%
add_header_above(c(" " = 1, "Besmettingen" = 2, "Overleden" = 2)) %>%
  footnote(number = c(footnote.leeftijd, footnote.under50),threeparttable = T)
``` 
...
sex_datalist <- list(sex.cases.today,sex.cases.weekago,sex.death.today,sex.death.weekago)
...
colnames(sex.data) <- c("statnaam","cases_today","cases_lastweek","death_today","death_lastweek")
...
sex.data.table <- sex.data[,c(1,7,9,8,10)]
colnames(sex.data.table) <- c("Geslacht","Totaal","%","Totaal","%")

sex.data.table$Geslacht <- as.character(sex.data.table$Geslacht)
sex.data.table[1,1] <- c("Vrouw")
sex.data.table[2,1] <- c("Man")

footnote.geslacht <- paste0("Man-vrouwverdeling van bij de GGD’en gemelde COVID-19 patiënten en overleden COVID-19 patiënten van ",format((Sys.Date()-14),"%d")," ",format((Sys.Date()-14),"%B"), " t/m ",format((Sys.Date()),"%d")," ",format((Sys.Date()),"%B")," 10:00 uur.")
